...
# define filefolder
# abideDir <- 'E:/ABIDE'
abideDir <- '/Volumes/Xueru/ABIDE'
dataDir <- file.path(abideDir, "Preprocessed")
analDir <- file.path(abideDir, "Analysis")
plotDir <- file.path(analDir, "Plot/gamlss/fewParcel")

# load in model
setwd(analDir)

# define parcels
ynames_group1 <- c("middletemporal", "medialorbitofrontal", "parstriangularis")
ynames_group2 <- c("precuneus", "superiortemporal", "superiorfrontal")

ynames <- c(ynames_group1, ynames_group2)
...
# add age transfer
age_trans <- read.csv("/Volumes/Xueru/ABIDE/Centile/bankssts.csv")[, 1:2]
...
asd.1.adi <- asd.1[, 59:62]
asd.2.adi <- asd.2[, 59:62]

write.csv(asd.1.adi, file.path(analDir, "asd.1.adi.csv"), row.names = F)
write.csv(asd.2.adi, file.path(analDir, "asd.2.adi.csv"), row.names = F)
################################## Part11:  full age #########################################
...
for (n in 1:length(ynames)) {
  ...
  ggplot(df, aes(x = x, y = y_ori, color = Group)) +
    ...
    geom_vline(xintercept = max(df$x), linetype = 2, lwd = 0.5, alpha = .3) + #年龄最大值
    geom_vline(xintercept = min(df$x), linetype = 2, lwd = 0.5, alpha = .3) + #年龄最小值
    geom_hline(yintercept = 0.5, linetype = 1, lwd = 0.5) +
    scale_x_continuous(limits = ageLimits, breaks = ageTicks,
  ...
  eval(parse(text = com_str <- paste0("name <- file.path(plotDir,'gamlss_age30.", yname,
                                      "_230526.png')")))
  ggsave(name, width = 6, height = 5, units = "in", dpi = 500)
...
# define
xnames <- c("middletemporal", "medialorbitofrontal", "parstriangularis")
ynames <- names(asd_all)[59:62]
...
for (m in 1:length(xnames)) {
  for (n in 1:length(ynames)) {
    for (s in 1:2) {
      ...
      data <- na.omit(data.frame(x_log, y))
      
      # 正态分布
      eval(parse(text = paste0("model.", s, " <- gamlss(y ~ pb(x_log), data = data, family = NO)")))
    ...
    # set age and color
    xTicks <- log(c(0.9, 0.7, 0.5, 0.3, 0.1))*(-1)
    xLimits <- log(c(0.9, 0.1))*(-1)
    colorTable <- c("#4682B4","#FFA07A")
    ...
    ggplot(df, aes(x = x, y = y, color = Group)) +
      ...
      scale_x_continuous(limits = xLimits, breaks = xTicks,
                         label = c("10%","30%","50%","70%", "90%"))+
      theme_cowplot() +
      xlab("") +
      ylab("") +
