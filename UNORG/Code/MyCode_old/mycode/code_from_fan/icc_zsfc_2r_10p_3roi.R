...
# 2 runs - 10p
workdir <- print(paste(filefolder,'2r_10p/',sep = ""));
setwd(workdir)
...
# visual
  ...
  step <- c(1:10);
  for (s in step){
    file <- print(paste("LT_S",s,"_v.csv",sep = ""))
    ...
    sigma_sub2 <- matrix(0,nrow=400,ncol=1);
    sigma_sub_vis2 <- matrix(0,nrow=400,ncol=1);
    sigma_err2 <- matrix(0,nrow=400,ncol=1);
    sigma_sub2_over_Vr <- matrix(0,nrow=400,ncol=1);
    sigma_sub_vis2_over_Vr <- matrix(0,nrow=400,ncol=1);
    sigma_err2_over_Vr <- matrix(0,nrow=400,ncol=1);
    ...
    iccfile <- print(paste(filefolder,"2r_10p/zSFC_ICC_LTs",s,"v_2.csv",sep = ""));
