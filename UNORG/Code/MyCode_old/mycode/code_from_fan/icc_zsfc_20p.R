...
# 1 run
workdir <- print(paste(filefolder,'1r/',sep = ""));
setwd(workdir)
  ...
  for (s in step){
    ...
    iccfile <- print(paste(filefolder,"1r/zSFC_ICC_LTs",s,".csv",sep = ""));
  ...
  filefolder <- print("/Users/fanxueru/Library/Mobile Documents/com~apple~CloudDocs/project/hcp/MasterProject/HCP_2visits_400ptseries/zSFC/");
  workdir <- print(paste(filefolder,'2r/',sep = ""));
  setwd(workdir)
  ...
  for (s in step){
    ...
    zSFCicc <- data.frame(ICC,sigma_sub2,sigma_sub_vis2,sigma_err2,Vr,sigma_sub2_over_Vr,sigma_sub_vis2_over_Vr,sigma_err2_over_Vr);
    iccfile <- print(paste(filefolder,"2r/zSFC_ICC_LTs",s,".csv",sep = ""));
    write.csv (zSFCicc, file =iccfile)
  }
  ...
  filefolder <- print("/Users/fanxueru/Library/Mobile Documents/com~apple~CloudDocs/project/hcp/MasterProject/HCP_2visits_400ptseries/zSFC/");
  workdir <- print(paste(filefolder,'4r/',sep = ""));
  setwd(workdir)
  ...
  for (s in step){
    ...
    for (i in 7:1206){
      ...
      T <- try({ mymodel <- lmer(sfc ~ age * gender + TestRetestInterval + (1 | idnew), data=newdata, REML=FALSE) });
      if ("try-error" %in% class(T))
      {
        next
      }
      ...
      sigma_sub2[i,1] <- a;
      sigma_err2[i,1] <- b;
      sigma_sub2_over_Vr[i,1] <- a/vr;
      sigma_err2_over_Vr[i,1] <- b/vr;
      
    }
    
    zSFCicc <- data.frame(ICC,sigma_sub2,sigma_err2,Vr,sigma_sub2_over_Vr,sigma_err2_over_Vr);
    iccfile <- print(paste(filefolder,"4r/zSFC_ICC_LTs",s,".csv",sep = ""));
    write.csv (zSFCicc, file =iccfile)
  }
