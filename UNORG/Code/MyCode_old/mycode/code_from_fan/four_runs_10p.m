...
for visit = 1:2;
    ...
    for sub = 1:41;
        ...
        LR_1 = timeseries; clear timeseries

        load(['sub',num2str(sub_id(sub,1)),'_rfMRI_REST'...
            ,num2str(rest),'_RL.ptseries.mat']);
        RL_1 = timeseries; clear timeseries
        ...
        load(['sub',num2str(sub_id(sub,1)),'_rfMRI_REST'...
            ,num2str(rest),'_LR.ptseries.mat']); %打开原始时间序列
        LR_2 = timeseries; clear timeseries

        load(['sub',num2str(sub_id(sub,1)),'_rfMRI_REST'...
            ,num2str(rest),'_RL.ptseries.mat']);
        RL_2 = timeseries; clear timeseries
        

        LR_RL_LR_RL = [LR_1;RL_1;LR_2;RL_2]; %把4个run纵向拼在一起
        z_LR_RL_LR_RL = atanh(LR_RL_LR_RL);%拼好的每个序列做fisher-z变换
        clear LR_1 RL_1 LR_2 RL_2 LR_RL_LR_RL rest

        cor = abs(corr(z_LR_RL_LR_RL)); %计算400个分区的相关并取绝对值
        ...
        for step = 1:10;
            ...
            cd (strcat([filefolder,'/Visit',num2str(visit),'/SFC_4r_10p/']));
...
for visit = 1:2;
    ...
    for step = 1:10;
        ...
        filename = strcat([filefolder,'/zSFC/4r_10p/S',num2str(step),...
...
filefolder = strcat(['/Users/fanxueru/Library/Mobile Documents/'...
    'com~apple~CloudDocs/project/hcp/MasterProject/'...
    'HCP_2visits_400ptseries/zSFC/4r_10p']);

for step = 1:10;
    
    zSFC_al = zeros(400,82);
    zSFC_sl = zeros(400,82);
    zSFC_vl = zeros(400,82);
    
    visit = 1;
    load(['S',num2str(step),'_V',num2str(visit),'_3roi.mat'])
    zSFC_vl(:,1:41) = vl;
    zSFC_al(:,1:41) = al;
    zSFC_sl(:,1:41) = sl;
    clear al sl vl
...
for s = 1:10;
    ...
    cd (strcat([filefolder_sfc,'/zSFC/4r_10p']))
    ...
    filefolder = strcat([filefolder_sfc,'/zSFC/4r_10p/nii/']);
...
cd (strcat(filefolder))
templet_1r_2r = dlmread('templet_4r.csv');

cd (strcat(filefolder,'/zSFC/4r_10p'))
...
for step = 1:10;
    ...
    visit = 1;
    load(strcat(['S',num2str(step),'_V',num2str(visit),'_3roi.mat']))
    ...
    AllRuns = [templet_1r_2r; V1 V2];
    clear rest visit V1 V2

    filename = strcat([filefolder,'/zSFC/4r_10p/for_cal_icc_S',num2str(step),'.mat']);
...
for s = 1:10;
    ...
    vl_sigma_sub2_over_vr = zSFCicc(1:400,5);
    vl_sigma_err2_over_vr = zSFCicc(1:400,6);
    vl_ICC = zSFCicc(1:400,1);

    al_sigma_sub2_over_vr = zSFCicc(401:800,5);
    al_sigma_err2_over_vr = zSFCicc(401:800,6);
    al_ICC = zSFCicc(401:800,1);

    sl_sigma_sub2_over_vr = zSFCicc(801:1200,5);
    sl_sigma_err2_over_vr = zSFCicc(801:1200,6);
...
for s = 1:10;  
    ...
    a.dscalar = vl_icc;
    ft_write_cifti(strcat([filefolder,'/zSFC/4r_10p/nii/vl_icc_LTs',num2str(s),...
        '.nii']),a,'parameter','dscalar');

    a.dscalar = vl_sub2;
    ft_write_cifti(strcat([filefolder,'/zSFC/4r_10p/nii/vl_sub2_LTs',num2str(s),...
        '.nii']),a,'parameter','dscalar');

    a.dscalar = vl_err2;
    ft_write_cifti(strcat([filefolder,'/zSFC/4r_10p/nii/vl_err2_LTs',num2str(s),...
        '.nii']),a,'parameter','dscalar');
    
    a.dscalar = al_icc;
    ft_write_cifti(strcat([filefolder,'/zSFC/4r_10p/nii/al_icc_LTs',num2str(s),...
        '.nii']),a,'parameter','dscalar');

    a.dscalar = al_sub2;
    ft_write_cifti(strcat([filefolder,'/zSFC/4r_10p/nii/al_sub2_LTs',num2str(s),...
        '.nii']),a,'parameter','dscalar');

    a.dscalar = al_err2;
    ft_write_cifti(strcat([filefolder,'/zSFC/4r_10p/nii/al_err2_LTs',num2str(s),...
        '.nii']),a,'parameter','dscalar');

    a.dscalar = sl_icc;
    ft_write_cifti(strcat([filefolder,'/zSFC/4r_10p/nii/sl_icc_LTs',num2str(s),...
        '.nii']),a,'parameter','dscalar');

    a.dscalar = sl_sub2;
    ft_write_cifti(strcat([filefolder,'/zSFC/4r_10p/nii/sl_sub2_LTs',num2str(s),...
        '.nii']),a,'parameter','dscalar');

    a.dscalar = sl_err2;
    ft_write_cifti(strcat([filefolder,'/zSFC/4r_10p/nii/sl_err2_LTs',num2str(s),...
        '.nii']),a,'parameter','dscalar');
