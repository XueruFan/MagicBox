# 画出普聚类分出的ASD男性两个亚型的34+7个脑指标的发育轨线+散点图（用百分位数建模）
...
plotDir <- file.path(abideDir, "Plot/Cluster/Cluster_A/SpectralCluster/CentileGAMM")
resDate <- "240315"
...
for (volumeName in volumeNames){
  ...
  ############################# asd个体的常模预测值
  # studyFIT$y <- studyFIT[, ynames[2]]
  # newFit <- expand.grid(AgeTransformed = ageRange)
  # fm <- gam(y ~ s(AgeTransformed, k = 5), data = studyFIT)
  # newFit$y <- predict(fm, newdata = newFit, se.fit = F)
  # plotData <- newFit
  ...
  name <- paste0("abide_A_asd_male_dev_Spectral_Cluster_", volumeName, "_GAMLSS_", resDate,".png")
