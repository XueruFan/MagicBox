# this script is used to do ABIDE 2 cluster (age 6-17.9 asd male only)
...
abide_centile <- read.csv(file.path(dataDir, paste0("abide_2_centile_dev_", date, ".csv")))
...
for (dx in dx_group){
  ...
  for (sex in sex_group){
    ...
    # save plot
    name <- file.path(plotDir, paste0("abide_2_", dx, "_pamK2_", sex, "_dev_Rank_", date, ".png"))
    ggsave(name, width = 8, height = 8, units = "in", dpi = 500)
    
    # save rank
    eval(parse(text = paste0("write.csv(sorted_feature_importance, file.path(resDir, 'abide_2_", dx,
    ...
    eval(parse(text = paste0("write.csv(pam_group, file.path(resDir, 'abide_2_", dx, "_pamK2_", sex,
