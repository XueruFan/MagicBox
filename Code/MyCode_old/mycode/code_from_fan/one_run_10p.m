...
for visit = 1:2;
    ...
    for rest = 1:2
        ...
        for step = 1:10;
            ...
            filename = strcat([filefolder,'/zSFC/1r_10p/S'...
...
cd (strcat(filefolder,'/zSFC/1r_10p'))
...
for step = 1:10;
    ...
    filename = strcat([filefolder,'/zSFC/1r_10p/for_cal_icc_S',num2str(step),'.mat']);
...
for s = 1:10;  
    ...
    a.dscalar = vl_icc;
    ft_write_cifti(strcat([filefolder,'/zSFC/1r_10p/nii/vl_icc_LTs',num2str(s),...
        '.nii']),a,'parameter','dscalar');
...
load(strcat(['S1_3roi.mat']));
Vis_S1 = mean_vl;

load(strcat(['S2_3roi.mat']));
% Aud_S2 = mean_al;
Som_S2 = mean_sl;

% AudS2_dele_VisS1 = Aud_S2-Vis_S1;
SomS2_dele_VisS1 = Som_S2-Vis_S1;
...
a.dscalar = draw;
ft_write_cifti(strcat([filefolder,'SomS2_dele_VisS1.nii']),a,'parameter','dscalar');
...
a = ft_read_cifti(strcat([filefolder_sfc,'/Schaefer2018_400Parcels_'...
 ...
 for v = 1:2; 
    for r = 1:2;
        DC_allsub = zeros(400,41);
        cd(strcat(filefolder_sfc,'/Visit',num2str(v),'/Rest',num2str(r),'/SFC_1r_10p'))
        for i = 1:41;
            load(['sub',num2str(i),'s1.mat'])
            dc = sum(sfc,2); 
            DC_allsub(:,i) = dc;
            dc_mean = mean(DC_allsub,2);

        end
        cd(strcat([filefolder_sfc,'/DC']))
        file = strcat(['V',num2str(v),'R',num2str(r),'dc.mat']);
        save(file,'DC_allsub','dc_mean')
    end
end
%%
