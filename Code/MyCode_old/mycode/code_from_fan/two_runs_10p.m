...
for visit = 1:2;
    ...
    for rest = 1:2;
        ...
        for sub = 1:41;
            ...
            LR = timeseries; clear timeseries

            load(['sub',num2str(sub_id(sub,1)),'_rfMRI_REST'...
                ,num2str(rest),'_RL.ptseries.mat']);
            RL = timeseries; clear timeseries

            LR_RL = [LR;RL]; %把2个run纵向拼在一起
            z_LR_RL = atanh(LR_RL);%拼好的每个序列做fisher-z变换
            clear LR RL LR_RL 

            cor = abs(corr(z_LR_RL)); %计算400个分区的相关并取绝对值
            ...
            for step = 1:10;
                ...
                clear sfc mean_sfc std_sfc
                
                cd (strcat([filefolder,'/Visit',num2str(visit),'/Rest'...
                    ,num2str(rest),'/SFC_2r_10p']));
...
for visit = 1:2;
    ...
    for rest = 1:2;
        ...
        for step = 1:10;
            ...
            filename = strcat([filefolder,'/zSFC/2r_10p/S',num2str(step),'_R',num2str(rest),...
...
filefolder = strcat(['/Users/fanxueru/Library/Mobile Documents/'...
    'com~apple~CloudDocs/project/hcp/MasterProject/'...
    'HCP_2visits_400ptseries/zSFC/2r_10p']);
...
for s = 1:10;
    ...
    cd (strcat([filefolder_sfc,'/zSFC/2r_10p']))
    ...
    filefolder = strcat([filefolder_sfc,'/zSFC/2r_10p/nii/']);
...
cd (strcat(filefolder,'/zSFC/2r_10p'))
...
for step = 1:10;
    ...
    AllRuns_v = [templet_1r_2r; V1R1_v V1R2_v V2R1_v V2R2_v];
    AllRuns_a = [templet_1r_2r; V1R1_a V1R2_a V2R1_a V2R2_a];
    AllRuns_s = [templet_1r_2r; V1R1_s V1R2_s V2R1_s V2R2_s];
    
    filename = strcat([filefolder,'/zSFC/2r_10p/for_cal_icc_S',num2str(step),'.mat']);
    save(filename,'AllRuns_v','AllRuns_s','AllRuns_a')
    
    csvwrite(['LT_S',num2str(step),'_v.csv'], AllRuns_v);
    csvwrite(['LT_S',num2str(step),'_a.csv'], AllRuns_a);
    csvwrite(['LT_S',num2str(step),'_s.csv'], AllRuns_s);
end
    % 建好3个roi的存储矩阵
%     V1R1 = zeros(1200,41);
%     V1R2 = zeros(1200,41);
...
% 
for s = 1:10;
    filename = strcat(['zSFC_ICC_LTs',num2str(s),'_v.csv']);
...
file = strcat(['LT_icc_v.mat']);
save(file,'vl_ICC_Step',...
    'vl_Sigma_sub2_over_vr_Step',...
    'vl_Sigma_sub_vis2_over_vr_Step',...
    'vl_Sigma_err2_over_vr_Step')
...
file = strcat(['LT_vr.mat']);
save(file,'vl_vr_Step')
...
%             end
    ...
    a.dscalar = vl_icc;
    ft_write_cifti(strcat([filefolder,'/zSFC/2r_10p/nii/vl_icc_LTs',num2str(s),...
        '_v.nii']),a,'parameter','dscalar');

    a.dscalar = vl_sub2;
    ft_write_cifti(strcat([filefolder,'/zSFC/2r_10p/nii/vl_sub2_LTs',num2str(s),...
        '_v.nii']),a,'parameter','dscalar');

    a.dscalar = vl_sub_vis2;
    ft_write_cifti(strcat([filefolder,'/zSFC/2r_10p/nii/vl_sub_vis2_LTs',num2str(s),...
        '_v.nii']),a,'parameter','dscalar');

    a.dscalar = vl_err2;
    ft_write_cifti(strcat([filefolder,'/zSFC/2r_10p/nii/vl_err2_LTs',num2str(s),...
        '_v.nii']),a,'parameter','dscalar');
    
%     a.dscalar = al_icc;
%     ft_write_cifti(strcat([filefolder,'/zSFC/2r_10p/nii/al_icc_LTs',num2str(s),...
%         '.nii']),a,'parameter','dscalar');
% 
%     a.dscalar = al_sub2;
%     ft_write_cifti(strcat([filefolder,'/zSFC/2r_10p/nii/al_sub2_LTs',num2str(s),...
%         '.nii']),a,'parameter','dscalar');
% 
%     a.dscalar = al_sub_vis2;
%     ft_write_cifti(strcat([filefolder,'/zSFC/2r_10p/nii/al_sub_vis2_LTs',num2str(s),...
%         '.nii']),a,'parameter','dscalar');
% 
%     a.dscalar = al_err2;
%     ft_write_cifti(strcat([filefolder,'/zSFC/2r_10p/nii/al_err2_LTs',num2str(s),...
%         '.nii']),a,'parameter','dscalar');
% 
%     a.dscalar = sl_icc;
%     ft_write_cifti(strcat([filefolder,'/zSFC/2r_10p/nii/sl_icc_LTs',num2str(s),...
%         '.nii']),a,'parameter','dscalar');
% 
%     a.dscalar = sl_sub2;
%     ft_write_cifti(strcat([filefolder,'/zSFC/2r_10p/nii/sl_sub2_LTs',num2str(s),...
%         '.nii']),a,'parameter','dscalar');
% 
%     a.dscalar = sl_sub_vis2;
%     ft_write_cifti(strcat([filefolder,'/zSFC/2r_10p/nii/sl_sub_vis2_LTs',num2str(s),...
%         '.nii']),a,'parameter','dscalar');
% 
%     a.dscalar = sl_err2;
%     ft_write_cifti(strcat([filefolder,'/zSFC/2r_10p/nii/sl_err2_LTs',num2str(s),...
%         '.nii']),a,'parameter','dscalar');
