...
setwd(filepath) # 设置路径
fn <- paste(p, "P_PCA_zdEC_all.mat",sep = "")
file <- paste0("zdEC.", dir, ".", value)
ec <- as.data.frame(t(readMat(fn)[[file]])); rm(file)
colnames(ec)[1:p] <- c(1:p); # 给分区命名
#建好待存入数据的矩阵
ICC <- matrix(0,nrow=p,ncol=1)
Vr <- matrix(0,nrow=p,ncol=1)
...
iccfile <- paste0(p, "P_PCA_zdEC_", dir, "_", value, "_ICC.mat")
